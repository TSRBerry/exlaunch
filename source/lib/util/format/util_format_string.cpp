    ...
    namespace
    {
        ...
        int TVSNPrintfImpl(char *const dst, const size_t dst_size, const char *format, ::std::va_list vl)
        {
            ...
            while (*format)
            {
                ...
                auto SetFlag = [&flags](FormatSpecifierFlag f) ALWAYS_INLINE_LAMBDA
                { flags |= f; };
                auto ClearFlag = [&flags](FormatSpecifierFlag f) ALWAYS_INLINE_LAMBDA
                { flags &= ~f; };
                auto HasFlag = [&flags](FormatSpecifierFlag f) ALWAYS_INLINE_LAMBDA
                { return (flags & f) != 0; };
                {
                    ...
                    while (!parsed_flags)
                    {
                        ...
                        {
                        case ' ':
                            SetFlag(FormatSpecifierFlag_EmptySign);
                            format++;
                            break;
                        case '+':
                            SetFlag(FormatSpecifierFlag_ForceSign);
                            format++;
                            break;
                        case '#':
                        ...
                        case '-':
                            ...
                            break;
                        case '0':
                            SetFlag(FormatSpecifierFlag_ZeroPad);
                            format++;
                            break;
                        default:
                            parsed_flags = true;
